# -*- coding: utf-8 -*-
"""
Created on Sun Aug 29 21:05:32 2021

@author: dchae2
"""

####WORD2VEC ANALYSIS ON YOUTUBE DATASET LEFT VS. RIGHT MEDIA BIAS
...
####creating wordlist for input of modeling in the next section
left = data[data['Bias'] == 'Left'].reset_index()
right = data[data['Bias'] == 'Right'].reset_index()

left = left['text'].tolist()
right = right['text'].tolist()
...
##### LEFT WORD2VEC MODELING

#Model Function
left_model = gensim.models.Word2Vec(left,\
                          workers=num_workers,\
...
left_model.init_sims(replace=True)

# Saving the model for later use. Can be loaded using Word2Vec.load()
left_model_name = "Sentiment Based Youtube WORD2VEC Model"
left_model.save(left_model_name)

#testing to see if the model properly works
left_model.wv.most_similar('illegal')
left_words = pd.DataFrame(model.wv.index_to_key)

#### RIGHT WORD2VEC MODELING

#Model Function
right_model = gensim.models.Word2Vec(right,\
                          workers=num_workers,\
                          vector_size = 145, #The vector_size shows the underlying dimension size 
                          min_count=min_word_count,\
                          window=context)
...
# To make the model memory efficient
right_model.init_sims(replace=True)

# Saving the model for later use. Can be loaded using Word2Vec.load()
right_model_name = "Sentiment Based Youtube WORD2VEC Model"
right_model.save(right_model_name)

#testing to see if the model properly works
right_model.wv.most_similar('illegal')
right_words = pd.DataFrame(right_model.wv.index_to_key)



############start constructing visualization

common = pd.read_csv('C:/Users/dchae2/Desktop/word2vec - Youtube/Top 30 pos.neg Common Words (Left.Right).csv')
common.columns = ['Left Negative', 'Left Positive', 'Right Negative', 'Right Positive']
LeftN = common['Left Negative'].apply(lambda x: tokenization(x.lower()))
LeftP = common['Left Positive'].apply(lambda x: tokenization(x.lower()))
RightN = common['Right Negative'].apply(lambda x: tokenization(x.lower()))
RightP = common['Right Positive'].apply(lambda x: tokenization(x.lower()))
...
def tsnescatterplot(model, word, lists, lists_2):
    ...
    reduc = PCA(n_components=61).fit_transform(arrays)
...
tsnescatterplot(left_model, 'vaccine', LeftN, LeftP)

tsnescatterplot(right_model, 'vaccine', RightN, RightP)
